...
public record RemoveRequestBody(int vehicleId)
    implements RequestBody<RemoveRequestBody.ResponseBody> {
    ...
    public ResponseBody getResponseBody(RequestBody.Context context) {
        try {
            DataSource.Vehicles.removeById(context.login(), vehicleId);
